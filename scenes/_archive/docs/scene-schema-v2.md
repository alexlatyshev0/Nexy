...
Multilingual field (en/ru) shown to users in the app. Must be:

1. **Brief but readable** - 1-2 short sentences max
2. **Natural language** - How people actually talk, not poetic/literary
3. **What's happening** - Describe the scene simply (sometimes include where)
4. **Key element(s)** - The essential aspect that makes this scene distinct

**Bad examples (too poetic, unnatural):**
```
"После интенсивности, нежность. Завёрнуты в одеяла, завёрнуты друг в друга. Спускаются вместе. Нежная забота после бури."
```

**Good examples (natural, clear):**
```
"Пара отдыхает после секса. Стараются быть нежными и ласковыми друг к другу."
"Он шлёпает её по попе. Она у него на коленях."
"Она делает ему минет. Глубоко, до горла."
```

### generation_prompt

Prompt for image generation (Civitai). Must follow rules:
1. Always specify genders explicitly (man/woman, not "person" or "partner")
2. Only ONE action per prompt (no sequences)
3. Always specify clothing state or nudity
4. Write spoken phrases if present
5. Never use speech bubbles
6. Minimize vague descriptions
7. Can mention lighting and setting
8. Never specify style in prompt (style handled via styleVariant)
